...
package com.example.bottombar.custombottom
...
internal typealias IBottomNavigationListener = (model: CustomBottomNavigation.Model) -> Unit

@Suppress("MemberVisibilityCanBePrivate")
class CustomBottomNavigation : FrameLayout {

    var models = ArrayList<Model>()
    var cells = ArrayList<CustomBottomNavigationCell>()
        private set
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.CustomBottomNavigation, 0, 0
        )
        try {
            a.apply {
                defaultIconColor = getColor(
                    R.styleable.CustomBottomNavigation_bn_defaultIconColor,
                    defaultIconColor
                )
                selectedIconColor = getColor(
                    R.styleable.CustomBottomNavigation_bn_selectedIconColor,
                    selectedIconColor
                )
                backgroundBottomColor = getColor(
                    R.styleable.CustomBottomNavigation_bn_backgroundBottomColor,
                    backgroundBottomColor
                )
                circleColor =
                    getColor(R.styleable.CustomBottomNavigation_bn_circleColor, circleColor)
                countTextColor =
                    getColor(R.styleable.CustomBottomNavigation_bn_countTextColor, countTextColor)
                countBackgroundColor = getColor(
                    R.styleable.CustomBottomNavigation_bn_countBackgroundColor,
                    countBackgroundColor
                )
                rippleColor =
                    getColor(R.styleable.CustomBottomNavigation_bn_rippleColor, rippleColor)
                shadowColor =
                    getColor(R.styleable.CustomBottomNavigation_bn_shadowColor, shadowColor)

                val typeface = getString(R.styleable.CustomBottomNavigation_bn_countTypeface)
                if (typeface != null && typeface.isNotEmpty())
                    countTypeface = Typeface.createFromAsset(context.assets, typeface)

                hasAnimation =
                    getBoolean(R.styleable.CustomBottomNavigation_bn_hasAnimation, hasAnimation)
            }
        } finally {
            a.recycle()
        }
    ...
    private fun initializeViews() {
        ...
        bezierView.apply {
            ...
            shadowColor = this@CustomBottomNavigation.shadowColor
        }
    ...
    fun add(model: Model) {
        val cell = CustomBottomNavigationCell(context)
        cell.apply {
            val params = LinearLayout.LayoutParams(0, heightCell, 1f)
            layoutParams = params
            icon = model.icon
            count = model.count
            defaultIconColor = this@CustomBottomNavigation.defaultIconColor
            selectedIconColor = this@CustomBottomNavigation.selectedIconColor
            circleColor = this@CustomBottomNavigation.circleColor
            countTextColor = this@CustomBottomNavigation.countTextColor
            countBackgroundColor = this@CustomBottomNavigation.countBackgroundColor
            countTypeface = this@CustomBottomNavigation.countTypeface
            rippleColor = this@CustomBottomNavigation.rippleColor
            onClickListener = {
                if (isShowing(model.id)) // added for https://github.com/shetmobile/CustomBottomNavigation/issues/39
    ...
    private fun anim(cell: CustomBottomNavigationCell, id: Int, enableAnimation: Boolean = true) {
        isAnimating = true
    ...
    fun getCellById(id: Int): CustomBottomNavigationCell? {
    ...
    fun clearCount(id: Int) {
        val model = getModelById(id) ?: return
        val pos = getModelPosition(id)
        model.count = CustomBottomNavigationCell.EMPTY_VALUE
        cells[pos].count = CustomBottomNavigationCell.EMPTY_VALUE
    }
    ...
    class Model(var id: Int, var icon: Int) {
        ...
        var count: String = CustomBottomNavigationCell.EMPTY_VALUE
